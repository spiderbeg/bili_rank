...
def get_new(send_count):
    ...
    logging.warning('更新后元组%s',send_count)
    newz = {} # 新的排名
...
logging.warning('输入的send_count%s',send_count)
start = time.time()
newz = get_new(send_count)
end = time.time()
use = end - start
logging.warning('原来的排名情况%s', oldz)
logging.warning('此次更新花费时间/分%s', use/60)
logging.warning('新的排名情况%s', newz)
...
logging.warning('时间：%s, 最后储存的排名情况%s', datetime.now(), usez)
for z in usez:
